      ...
      ProposalCreateRequest: {
        ...
        properties: {
          ...
          clientRequestKey: {
            type: "string",
            format: "uuid",
            description: "Idempotency key generated by the client for safe retries.",
          },
          type: {
            type: "string",
            enum: ["TRAVELER_OFFER", "REQUESTER_REQUEST"],
          },
          message: { type: "string", minLength: 1, maxLength: 500, nullable: true },
        },
      },
      Proposal: {
        type: "object",
        properties: {
          ...
          type: { type: "string", enum: ["TRAVELER_OFFER", "REQUESTER_REQUEST"] },
          status: { type: "string", enum: ["PENDING", "ACCEPTED", "REJECTED", "WITHDRAWN", "EXPIRED"] },
          message: { type: "string", nullable: true },
          rejectionReason: {
            type: "string",
            nullable: true,
            enum: ["REJECTED_BY_OWNER", "ANOTHER_PROPOSAL_ACCEPTED", "TRIP_CAPACITY_FULL"],
          },
  ...
  paths: {
    ...
    "/api/v1/proposals": {
      post: {
        ...
        responses: {
          ...
          200: { description: "Identical retry returned the existing proposal." },
          400: { $ref: "#/components/responses/ValidationFailed" },
          401: { $ref: "#/components/responses/Unauthorized" },
          403: errorResponse("Only the required errand or trip owner can create this proposal type."),
    ...
    "/api/v1/errands/{id}/proposals": {
      get: {
        ...
        responses: {
          200: { description: "Errand proposals and status summary retrieved successfully." },
          400: { $ref: "#/components/responses/ValidationFailed" },
          401: { $ref: "#/components/responses/Unauthorized" },
          403: errorResponse("Only the errand owner can view incoming proposals."),
          404: errorResponse("Errand not found."),
        },
      },
    },
    "/api/v1/trips/{id}/proposals": {
      get: {
        ...
        responses: {
          200: { description: "Trip proposals and status summary retrieved successfully." },
          400: { $ref: "#/components/responses/ValidationFailed" },
          401: { $ref: "#/components/responses/Unauthorized" },
          403: errorResponse("Only the trip owner can view incoming proposals."),
          404: errorResponse("Trip not found."),
        },
      },
    },
    "/api/v1/proposals/{id}/accept": {
      post: {
        ...
        responses: {
          200: { description: "Proposal accepted and assignment created successfully." },
          401: { $ref: "#/components/responses/Unauthorized" },
          403: errorResponse("Only the proposal receiver can accept it."),
          404: errorResponse("Proposal not found."),
          409: errorResponse("Proposal is not pending, expired, or the errand is no longer available."),
        },
      },
    },
    "/api/v1/proposals/{id}/reject": {
      post: {
        ...
        description: "Receiver-only action. Stores REJECTED_BY_OWNER and does not charge tokens or affect other errands.",
        security: [{ bearerAuth: [] }],
        parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
        responses: {
