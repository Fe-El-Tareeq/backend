...
const validImageContent = (file) => {
  ...
  return (file.mimetype === "image/jpeg" && jpeg) ||
    (file.mimetype === "image/png" && png) ||
    (file.mimetype === "image/webp" && webp);
};

const uploadIdentityVerification = (req, res, next) => {
  upload.fields(requiredFields.map((name) => ({ name, maxCount: 1 })))(req, res, (error) => {
    ...
    if (requiredFields.some((name) => !files[name]?.[0])) {
      return next(new ApiError(400, "Front ID, back ID, and selfie images are required."));
    }
    if (requiredFields.some((name) => !validImageContent(files[name][0]))) {
      return next(new ApiError(400, "One or more uploaded files are not valid images."));
    }
    return next();
  });
};
